...
兼容层仍接受旧式纯 `<tool_calls>` wrapper，并会容错若干 DSML 标签变体，包括短横线形式 `<dsml-tool-calls>` / `<dsml-invoke>` / `<dsml-parameter>`，以及 DeepSeek-V4 有时会在标签外加 Markdown 加粗标记的变体（如 `<**DSML|tool_calls**>`、`<*DSML|invoke*>`、`</**DSML|parameter**>`）；scanner 和 normalizer 会自动跳过 `*` 标记，确保这些变体也能被正确识别为工具调用。此外，DeepSeek-V4 在加粗标记场景下还会产生嵌套裸 parameter 变体（如 `<**DSML|parameter name="x"**><**DSML|parameter>value</**DSML|parameter**>`），兼容层会在去除 `*` 后自动删除无属性的裸 `<|DSML|parameter>` 标签，将其展平为标准的 `<|DSML|parameter name="x">value</|DSML|parameter>` 格式。此外，DeepSeek-V4 偶尔会在 DSML 标签中使用全角 Unicode 字符：用 `｜`（U+FF5C 全角竖线）替代 `|`，用 `▁`（U+2581 下横线）替代 `_`，产生如 `<｜DSML▁tool_calls｜>` 的变体；scanner 和 normalizer 会自动将这些全角字符归一为对应的 ASCII 字符，确保工具调用仍能被正确识别和解析。但提示词会优先要求模型输出官方 DSML 标签，并强调不能只输出 closing wrapper 而漏掉 opening tag。需要注意：这是"兼容 DSML 外壳，内部仍以 XML 解析语义为准"，不是原生 DSML 全链路实现；DSML 标签会在解析入口归一化回现有 XML 标签后继续走同一套 parser。
